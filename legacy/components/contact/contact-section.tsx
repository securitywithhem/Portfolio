  ...
  return (
    <SectionPremium id="contact" spacing="spacious" accentLine="top">
      ...
      <div className="mx-auto w-full max-w-2xl">
        <p className="text-text-secondary mb-12 text-center text-lg leading-relaxed">
        ...
        {directContact.length > 0 && (
          <dl className="border-border-subtle mt-16 grid grid-cols-1 gap-px overflow-hidden border-y sm:grid-cols-2">
            {directContact.map((item) => (
              <div key={item.label} className="bg-bg-base px-5 py-5">
                <dt className="text-text-muted font-mono text-[11px] font-medium tracking-[0.12em] uppercase">
                  {item.label}
                </dt>
                <dd className="text-text-primary mt-1.5 text-sm">
                  {item.href ? (
                    <a
                      href={item.href}
                      target={
        ...
        <p className="text-text-secondary mt-12 border-l-2 border-accent pl-6 text-base leading-relaxed italic">
